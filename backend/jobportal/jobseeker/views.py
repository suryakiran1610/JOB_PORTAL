    ...
    def get(self, request):
        ...
        if remaining_limit > 0:
            if not keywords and not location:
                ...
                additional_offset = max(0, start_index + len(jobs) - filtered_jobs_count)
                additional_jobs = jobpost.objects.exclude(filters).order_by('-jobposteddate')[additional_offset:additional_offset + remaining_limit]
