...
function Viewjobdetails() {
  ...
  const Applyjob = async () => {
    ...
    if (addresume) {
      ...
      try {
        const response = await fetch(`${config.imagebaseurl}${profile.resume}`);
        const blob = await response.blob();
        const file = new File([blob], "existing_resume.pdf", { type: blob.type });
        formData.append("resume", file);
      } catch (error) {
        console.error("Error fetching the existing resume:", error);
        setMessage("Error fetching the existing resume");
        return;
      }
    }
                ...
                <div className="p-2">
                  ...
                  <div className="flex items-center mb-4">
                    <input
                      ...
                      value="existing"
                      checked={addresume!=""}
                      onChange={() => setAddresume(null)}
                      className="mr-2"
                    />
                    <label htmlFor="existingResume">Use existing resume</label>
                  </div>
                  ...
                  <div className="flex items-center mb-4">
                    <input
                      ...
                      value="new"
                      checked={addresume==""}
                      onChange={() => setAddresume("")}
                      className="mr-2"
                    />
                    <label htmlFor="newResume">Upload new resume</label>
                  </div>
                  {addresume === "" && (
                    <input
