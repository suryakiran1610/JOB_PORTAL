...
function Companyinfo({ setActiveComponent,setCompanyName }) {
    ...
    function HandleCompanyInfo(e) {
        ...
        } else if (name === 'mobile' && (value.length !== 10 || !/^\d+$/.test(value))) {
            newErrors[name] = "Mobile number must be 10 digits";
        } else if (name === 'pin_code' && (value.length !== 6 || !/^\d+$/.test(value))) {
            newErrors[name] = "Pin code must be 6 digits";
        } else {
    ...
    const validateFields = () => {
        ...
        if (companyinfo.mobile.length !== 10 || !/^\d+$/.test(companyinfo.mobile)) {
            newErrors.mobile = "Mobile number must be 10 digits";
        }
        if (companyinfo.pin_code.length !== 6 || !/^\d+$/.test(companyinfo.pin_code)) {
    ...
    return (<>
            <form onSubmit={handleSubmit} encType="multipart/form-data" className="flex justify-evenly pt-10 max-sm:flex-col-reverse max-sm:justify-normal max-sm:pt-3" >
                < div className='flex flex-col gap-2 max-sm:p-8 ml-5'>
                    ...
                    <label className='flex flex-col mb-1 gap-1 text-xs'>
                        ...
                        <input
                            ...
                            className='signup-input border border-gray-300 w-full h-8 p-2'
                        />
                    ...
                    Continue <FontAwesomeIcon className='text-white continue-btn' icon={faArrowRight} color='white' />
                    </button>
                </div >
                <div className="flex flex-col gap-2 ml-9 max-sm:ml-5 max-sm:mt-4 max-sm:p-8 ">
                    <div className='font-bold  text-base mb-3'>
                        ...
                        <input
                         type="file" 
                         title="" 
                         className='choose-file-box-company'
